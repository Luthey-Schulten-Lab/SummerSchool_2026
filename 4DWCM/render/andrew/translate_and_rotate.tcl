...
set ribo_rad 70.0
set bdry_rad 65.0
# bdry radius formerly 32.5

# set the file to be loaded
set molecule_file ../trajectory/Mar31_1/DNA/chromosome.lammpstrj
...
set DNA2 [atomselect $mol_id "vy>2 and index < 54338" frame now]
...
mol modcolor $DNA_rep $mol_id ColorID 12
mol colupdate $DNA_rep $mol_id on

# create representation for DNA
mol addrep $mol_id
set DNA2_rep $rep_count
set rep_count [expr $rep_count + 1]
mol modstyle $DNA2_rep $mol_id VDW 13.1 12.0
mol modmaterial $DNA2_rep $mol_id AOChalky
mol modselect $DNA2_rep $mol_id [$DNA2 text]
mol scaleminmax $mol_id $DNA_rep 1 [$DNA2 num]
mol selupdate $DNA2_rep $mol_id on
mol modcolor $DNA2_rep $mol_id ColorID 27
mol colupdate $DNA2_rep $mol_id on
...
# Define the transformation matrix to swap x and z
set swap_matrix [list [list 0 0 1 0] \
                     [list 0 1 0 0] \
                     [list 1 0 0 0] \
                     [list 0 0 0 1]]

# Apply the matrix transformation to swap x and z for all frames
for {set i 0} {$i < $nframes} {incr i} {
    $all frame $i
    $all move $swap_matrix
}
...
# Loop through each frame and apply the displacement
for {set i 0} {$i < $nframes} {incr i} {
    $all frame $i
    $all moveby "$dx $dy $dz"
}
