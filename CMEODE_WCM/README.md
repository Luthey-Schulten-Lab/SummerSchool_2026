...
In the CME-ODE WCM tutorial, you will learn how to use hybrid algorithm to simulate the **Coupled Genetic Information Processes and Metabolism in the Minimal Cell**. 

The essential metabolism[^breuer_metabolism] in Syn3A imports nutrients from the growth medium and further converts them to generate ATP molecules, which energize cellular processes, and monomers for the synthesis of proteins, RNAs, and the chromosome. To simulate the co-evolution of GIP and metabolism in Syn3A, we employ a hybrid stochastic-deterministic algorithm[^bianchi_CMEODE], where stepwise communication describes the interactions between these two subsystems.
