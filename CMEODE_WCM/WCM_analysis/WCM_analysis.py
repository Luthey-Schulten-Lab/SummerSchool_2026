...
import numpy as np
from numpy.lib.format import open_memmap # Write out npy file
...
import re
try:
    import psutil  # optional: only used to report RAM usage during read
except ImportError:
    psutil = None
from concurrent.futures import ProcessPoolExecutor, as_completed # READ IN CSV files in parallel
...
def _read_traj_worker(args):
    """Read one trajectory CSV in a separate process; returns (i_rep, t, x, fx).

    Top-level (picklable) so ProcessPoolExecutor can dispatch it. The CSV readers
    (numpy loadtxt) hold the GIL and are memory-bound, so a thread pool does not
    parallelize them (measured: threads were slower than serial). Separate
    processes have independent GILs and parallelize the text->float parse; each
    worker returns one replicate's arrays and the parent assembles them.
    """
    i_rep, traj_file = args
    traj = WCMt.WCM_traj(traj_file)
    return i_rep, traj.get_t(), traj.get_x(), traj.get_fx()
...
class WCM_ensemble:
    ...
    def write_merged_ensemble(self, out_dir, out_label):
        ...
        x_path = os.path.join(out_dir, out_label + '_x.npy')
        fx_path = os.path.join(out_dir, out_label + '_fx.npy')

        # Stream-write to disk without full serialization buffer
        x_memmap = open_memmap(x_path, mode='w+', dtype=np.float32, shape=self.x.shape)
        x_memmap[:] = self.x[:]  # no RAM duplication

        fx_memmap = open_memmap(fx_path, mode='w+', dtype=np.float32, shape=self.fx.shape)
        fx_memmap[:] = self.fx[:]

        del x_memmap, fx_memmap

        del self.x, self.fx
        # # Save large arrays separately
        # np.save(os.path.join(out_dir, out_label + '_x.npy'), self.x)
        # np.save(os.path.join(out_dir, out_label + '_fx.npy'), self.fx)
        ...
        # Use stdlib pickle (protocol 4) rather than joblib: the ensemble then
        # loads in any env with numpy (no joblib dependency) and stays readable
        # by Python 3.7 (the LM 2.5 analysis kernel). Big arrays stay in .npy.
        with open(out_file, 'wb') as fh:
            pickle.dump(out_ensemble, fh, protocol=4)

        return

    def read_merged_ensemble(self, in_dir, in_label):
        
        # Record RAM usage (psutil is optional)
        ...
        # pickle.load (not joblib): works in any env with numpy, no joblib needed
        with open(in_file, 'rb') as fh:
            in_ensemble = pickle.load(fh)
    ...
    # def load_trajs(self):
    # Muted due to the high RAM usage
    # Changed to load per traj inside merge_trajs()
    #     self.trajs = []
    ...
    def merge_trajs(self, max_workers):

        # Step 1: determine the common set of timesteps and species
        print(f"Assuming that all cell replicates have the same time line, species, and reactions.")

        traj = WCMt.WCM_traj(self.traj_files[0])
        self.t = traj.get_t()
        ...
        # self.t = np.intersect1d(self.t,traj.get_t()) # Find the intersection
                
        # # Step 2: Allocate memory for x and fx
        ...
        # Step 3: Fill in x and fx from each trajectory
        print(f"Start merging {self.N_reps} CSV files using up to {max_workers} worker processes")
        # def read_and_copy(i_rep):
        #     traj = WCMt.WCM_traj(self.traj_files[i_rep])
        #     t_rep = traj.get_t()
        #     x_rep = traj.get_x()
        #     fx_rep = traj.get_fx()

        #     _, _, t_rep_idx = np.intersect1d(self.t, t_rep, return_indices=True)

        #     return i_rep, x_rep[:, t_rep_idx], fx_rep[:, t_rep_idx]

        # print(F"Using {max_workers} CPU cores to accelerate the READ IN of CSV files")
        # with ThreadPoolExecutor(max_workers=max_workers) as executor:
        #     futures = [executor.submit(read_and_copy, i) for i in range(self.N_reps)]
        #     for future in as_completed(futures):
        #         i_rep, x_slice, fx_slice = future.result()
        #         self.x[:, :, i_rep] = x_slice
        #         self.fx[:, :, i_rep] = fx_slice

        # set the elements of the new array, one replicate per WORKER PROCESS.
        # Each worker reads+parses one trajectory CSV (GIL-bound, so threads do
        # not help) and returns its arrays; the parent assembles them into the
        # ensemble. ~2.5x over the serial loop on 8 replicates, byte-identical.
        n_workers = max(1, min(int(max_workers), self.N_reps))

        if n_workers == 1:
            # serial fallback (also avoids process overhead for a single rep)
            for i_rep in range(self.N_reps):
                _, t_rep, x_rep, fx_rep = _read_traj_worker((i_rep, self.traj_files[i_rep]))
                _, _, t_rep_idx = np.intersect1d(self.t, t_rep, return_indices=True)
                self.x[:,:,i_rep] = x_rep[:,t_rep_idx]
                self.fx[:,:,i_rep] = fx_rep[:,t_rep_idx]
        else:
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                futures = [executor.submit(_read_traj_worker, (i, self.traj_files[i]))
                           for i in range(self.N_reps)]
                # as_completed: consume each ~200 MB result as soon as it is ready
                # so at most ~n_workers replicate arrays are buffered at once.
                for future in as_completed(futures):
                    i_rep, t_rep, x_rep, fx_rep = future.result()
                    _, _, t_rep_idx = np.intersect1d(self.t, t_rep, return_indices=True)
                    self.x[:,:,i_rep] = x_rep[:,t_rep_idx]
                    self.fx[:,:,i_rep] = fx_rep[:,t_rep_idx]
        
        print(f'Size of the pkl file: ({self.N_species, self.Nt, self.N_reps}')
        print('Non-Flux Array:', self.x.shape)
        print('Flux Array:', self.fx.shape)
