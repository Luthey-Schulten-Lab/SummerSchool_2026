...
def read_params_file(params_file):
    ...
    AA_params = pd.read_excel(params_file,sheet_name='tRNA Charging')
...
def get_params_comparison(old_params_file, new_params_file):
    """
    Description:
        Compare two param files in metabolic constants
    Return:
        Dictionary with key as the reaction name and value the differences, e.g. {'Eff Enzyme Count':[old, new], 'GPR rule':[old, new]}

    """

    print("Compare two kinetic files")
    print(f"NEW: {new_params_file}")
    print(f"OLD: {old_params_file}")
    ...
    for rxn in intersect_rxns:
        ...
        for index, row in new_temp.iterrows():
            ...
            compared_paramter_types = ['Substrate Catalytic Rate Constant',
            ...
            if (new_param_type not in compared_paramter_types):

                need_species = True
                new_species = row['Related Species']
                old_entry = old_entry[old_entry['Related Species'] == new_species]

            old_val = old_entry['Value'].iloc[0]
            new_val = row['Value']

            if old_val != new_val:
                params_diff[rxn][new_param_type] = [old_val, new_val]
    


    return old_params, new_params, params_diff
