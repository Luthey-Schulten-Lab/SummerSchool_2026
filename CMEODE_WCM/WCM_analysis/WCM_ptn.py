...
def get_ribosome(pkl, locations_ptns, YidC_ptns):
    """
    Get the counts of ribosome in different states
    """
    
    LocusNums2RNCs = getLocusNumRNCs(locations_ptns, YidC_ptns)

    Location2RNCs = getLocation2RNCs(locations_ptns, LocusNums2RNCs, YidC_ptns)

    free_ribo = pkl.get_specie_trace('Ribosome')

    cyto_ribo_species, SRP_ribo_species, SecA_ribo_species = Location2RNCs['Cytoplasmic'], Location2RNCs['SRP'], Location2RNCs['SecA']

    cyto_ribo = np.sum(pkl.get_species_traces(cyto_ribo_species), axis=0)
    SRP_ribo = np.sum(pkl.get_species_traces(SRP_ribo_species), axis=0)
    SecA_ribo = np.sum(pkl.get_species_traces(SecA_ribo_species), axis=0)

    active_ribo = cyto_ribo + SRP_ribo + SecA_ribo

    return free_ribo, active_ribo, cyto_ribo, SRP_ribo, SecA_ribo

def get_RNAP(pkl, TypetoLocusNums):
    ...
    RNAP_core_count = pkl.get_specie_trace('RNAP_core')
    free_RNAP_count = pkl.get_specie_trace('RNAP') + RNAP_core_count

    # Active RNAP

    RNAP_bound_ids = ['RNAP_G_' + locusNum for locusNum in genes]

    RNAP_bound_counts = pkl.get_species_traces(RNAP_bound_ids)
...
def get_degradosome(pkl, TypetoLocusNums):

    free_degra_count = pkl.get_specie_trace('Degradosome')
    ...
    degra_bound_counts = pkl.get_species_traces(degra_bound_ids)
...
def correctInitPtnCount(PtnIniCount, locusNum, cplx_dict, ribosomal_ptn_list, print_flag=False):
    ...
    if locusNum in ribosomal_ptn_list:
        stoi = cplx_dict['Ribosome']['Stoi'][locusNum]
        corrected_PtnIniCount = max(25, int(PtnIniCount - stoi*500))
        complex.append('Ribosome')
        total_PtnIniCount = corrected_PtnIniCount + 500*stoi
        if print_flag:
    ...
    for locusNum in locusNums:
        if locusNum in locations_ptns['peripheral membrane']+locations_ptns['cytoplasm']+locations_ptns['unidentified']:
            ...
            if locusNum in YidC_ptns:
                ...
                RNC_prefixes = ['RNC_', 'SRP_RNC_', 'SR_SRP_RNC_', 'SecYEGDF_SR_SRP_RNC_', 'SecYEGDF_RNC_', 'RNC_Long_']
...
def getLocation2RNCs(locations_ptns, LocusNums2RNCs, YidC_ptns):
    ...
    SecA_LocusNums = locations_ptns['extracellular'] + locations_ptns['lipoprotein']

    SRP_LocusNums = list(set(locations_ptns['trans-membrane']) - set(YidC_ptns))
    
    print(f"Total {len(cyto_LocusNums+SecA_LocusNums+SRP_LocusNums)} protein are covered in getLocation2RNCs function")
...
def get_bottleneck_species(w, TypetoLocusNums, stois, locusNums, cplx):
    ...
    gene_names = gene.get_gene_names('/data/enguang/CMEODE/SIM/input_data/initial_concentrations.xlsx', bottle_neck_locusNums, w.genomeDict)
