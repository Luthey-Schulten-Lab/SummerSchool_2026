...
def getLSUInters(LSUfile_dir):
    ...
    assemblyRxns = pd.read_excel(LSUfile_dir, sheet_name='LSU Assembly', index_col=None)
...
def getCountAvailable(w, IDsToLocusNums, SSULSUflag, Stoi):
    """
    Stoi: stoichiometries with key as locusNum, value as stoi
    """
    ...
    for ID, LocusNums in IDsToLocusNums.items():
            ...
            if ID.startswith(initialLetter): #r-proteins
                    ...
                    temp = temp/Stoi[locusNum] # for L7/L12 with four copies
                    # if locusNum == '0806':
                    #     print(temp)
